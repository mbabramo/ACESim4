...
/// <summary>Three risk-neutral illustrative rows; the complete packet remains online.</summary>
public static class SelectedEquilibriumManuscriptTable
{
    ...
    public static string Latex(Panel[] panels)
    {
        foreach (var panel in panels)
        {
            ...
            if (panel.Rows.Any(r => r.CounterfactualUndefined || r.UnreachedCoalitions.Length != 0 ||
                Math.Abs(r.Allocation.Exit) > PolicyTolerance || Math.Abs(r.Allocation.SelectionResidual) > PolicyTolerance))
                throw new InvalidDataException("Revisit the short table: omitted columns or reach qualifications are needed.");
        }
        if (panels.Sum(p => EquilibriumPublicationTables.GroupRows(p.Rows).Length) != 3)
            throw new InvalidDataException("The short table must contain three displayed rows; revisit changed grouping.");
        ...
        for (int i = 0; i < panels.Length; i++)
        {
            ...
            b.AppendLine(@"\multicolumn{6}{@{}l}{\textit{" + (char)('A' + i) + ". " +
            ...
            foreach (var group in EquilibriumPublicationTables.GroupRows(panels[i].Rows))
            {
                ...
                string label = row.Decision switch {
                    ...
                    "P Offer" => "P demand, continue", _ => throw new InvalidDataException("Unexpected selected decision.") };
                ...
                foreach (double effect in new[] { a.Direct, a.Entry, a.Offers })
    ...
    {
        var panels = Select(full);
        string tex = Path.Combine(input, "Sources", "Tex", Stem + ".tex");
        await File.WriteAllTextAsync(tex, Latex(panels));
        await File.WriteAllTextAsync(Path.Combine(input, "Sources", "Json", Stem + ".json"),
            JsonSerializer.Serialize(new {
                Schema = "1", Caption, Cost = 1,
                Selection = "Three risk-neutral illustrative rows selected for manuscript discussion; complete comparison packet retained online",
                OmittedZeroColumns = new[] { "Opponent exit", "Remaining" },
