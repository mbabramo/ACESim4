...
namespace LitigCharts
{
    public static class Runner
    {
        ...
        public static void ProcessLitigationGameData(DataBeingAnalyzed article)
        {
            ...
            bool buildMainReport = false; // DEBUG true; // this looks at all of the csv files containing the report outputs (e.g., Report Name.csv where there is only one equilibrium, or "-eq1", "-eq2", "-Avg", etc.), and then aggregates all of the information on the report outputs for each simulation into a CSV file, including both All cases and separate rows for various subsets of cases. Set this to false only if it has already been done. 
            bool printIndividualLatexDiagrams = false; // DEBUG true; // this is the time consuming one -- it applies to the heat map and offers diagrams for each individual equilibrium
