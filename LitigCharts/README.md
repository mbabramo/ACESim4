...
The historical descriptive publication tables use a separate saved-data command (the current five-measure welfare tables are generated by the diagram workflow described below):
...
| dispositions | With WelfareExhibitsRequest, generate separate welfare tables and disposition charts at every saved cost/specification. Otherwise use the legacy selected bar chart. |
| publication | Generate selection-offers plus the welfare tables and dispositions. |
| welfare-outcomes | Generate only the per-cost welfare tables and disposition charts, including extensions. |
...
| results | Individual, multiple-equilibrium, aggregate, and welfare/disposition targets. |
...
The article configuration checks 804 individual, 216 multiple-equilibrium, and 338 aggregate diagrams.
Welfare/disposition counts are discovered from all saved cases in the configured input batches;
the current 144 cases form 56 groups and produce 112 separate exhibits. Use `--list` for the total
selected by a target. These counts describe available outputs, not the revision-plan figure selection.
...
The article repository's configuration also selects CenterWeightedContinuousMerits
and PolarizedContinuousMerits, with filename prefixes `Center-weighted merits`
and `Polarized merits`. They use their existing production option sets and generate
the same forward, inverse and party-to-party relationships as the baseline.
LowNoise and HighNoise remain optional entries. To generate additional variants
without adding files to the article, use a separate config and --output-root.
...
The article configuration now sets `WelfareExhibitsRequest` to `Results/welfare-exhibits.json`.
This replaces the fixed main disposition request and the old `AdditionalDispositionFigures` selections
with automatically paired exhibits for all available costs and extensions. Outputs are under
`Results/Welfare outcomes and dispositions/Baseline` and `Extensions/<specification>`.
`diagrams results`, `all`, `publication`, and `dispositions` include both families automatically;
`diagrams welfare-outcomes` regenerates just these exhibits. A separate `welfare-outcomes --request`
command uses the same generator when no full diagram configuration is needed.

`WelfareOutcomeExhibits` reads each source's numerical summary and matching individual reports.
It discovers cost levels and specifications rather than maintaining a separate selection for each figure.
RN/RA baseline cases share a group, as do low-noise RN/RA cases; other specifications and offer grids
remain distinct. Only available fee rules are displayed; complete fee shifting is never imputed for
an extension. Add one Inputs entry for a new report batch. The existing reporting workflow must first
produce/update those configured summaries and individual reports after equilibrium determination.
The diagram command never invokes the solver or edits those source reports.

Each `welfare-outcomes-cost-<cost>.pdf` is a five-measure table; each `dispositions-cost-<cost>.pdf`
is one bar chart. Neither contains a title, cost annotation, caption, or explanatory paragraph.
There are no combined packets. Table numeric headings and values are centered. Both formats use
American, Trial Fee-Shifting, and Complete Fee-Shifting. PNG, standalone TeX, exact-data JSON,
CSV, calculation audits, and source/output hashes accompany the exhibits; definitions and coverage
are in the generated README. All headline quantities use all potential disputes.

Gross outcome error is computed from truth-specific base payments, excluding real expenditures and
separately awarded fee transfers. The calculation includes nonfiling and allocates mutual give-up once.
Stage-cost checks use each specification's actual cost timing. Numerical and accounting validation
