...
namespace ACESim
{
    public class CurveFitting5DGame : Game
    {
        ...
        public override void PrepareForOrMakeCurrentDecision()
        {
            ...
            CurveFitting5DGameInputs CurveGameInputs = (CurveFitting5DGameInputs) GameInputs;
            CurveFitting5DGameProgressInfo CurveGameProgress = (CurveFitting5DGameProgressInfo)Progress;
            ...
            switch (CurrentDecisionIndex)
            {
                case 0:
                    ...
                    // NOTE: When switching among number of inputs, must change CurveFitting5DGameInputs.cs and getDecisionInputs.
                ...
                //double target = CurveGameInputs.x * 4 / (CurveGameInputs.y * CurveGameInputs.y + 0.1 * CurveGameInputs.x);
                   // double target = 25 * (CurveGameInputs.x - 1.4) * (CurveGameInputs.y + 1.2) * (CurveGameInputs.z + 4.2) / (CurveGameInputs.v * CurveGameInputs.w * (double)Math.Sqrt((double)CurveGameInputs.w) + 8);
                    //double target = (CurveGameInputs.x + CurveGameInputs.y/3.0 + CurveGameInputs.z + CurveGameInputs.v + CurveGameInputs.w) / 5.0;
                    
                    double target = 2.3 / (2 + (Math.Pow(Math.Abs(CurveGameInputs.x - 0.3),1.2) + (Math.Pow(Math.Abs(CurveGameInputs.y - 0.3), 1.2)))) + 2.3 / (2 + (Math.Pow(Math.Abs(CurveGameInputs.y - 0.5), 1.5) + (Math.Pow(Math.Abs(CurveGameInputs.z - 0.4), 1.2)))) * 4 / (2 + (Math.Pow(Math.Abs(CurveGameInputs.v - 0.3), 1.2))) + (2.4 / (1.5 + (Math.Pow(Math.Abs(CurveGameInputs.w - 0.4), 1.2)) * (2 / (3 + (Math.Pow(Math.Abs(CurveGameInputs.x - 0.9), 2.8)))))) - 3;
                ...
                // double target = (CurveGameInputs.x > 2.5) ? 20 : 10;
                    ...
                    //double target = (CurveGameInputs.x + (CurveGameInputs.z*(CurveGameInputs.v + 0.5) - 0.5)) + CurveGameInputs.y + CurveGameInputs.w; // this is an example where we should be able to easily group the inputs into a tree
                    //// x + y then w
                    //// |
                    //// x = x + (z - 0.5)
                    ////          |
                    ...
                    //    target *= 4;
                    //double target = (5 / (1 + 3 * (CurveGameInputs.x - 3) * (CurveGameInputs.x - 3)))*(1 / (1 + 2 * Math.Min((CurveGameInputs.y - 3),0)*Math.Min((CurveGameInputs.y - 3),0)));
                    ...
                    CurveGameProgress.Target = target;
                    CurveGameProgress.Guess = guess;
                ...
                case 1:
                    ...
                    //CurveGameProgress.TotalScoreDecision2 = Math.Abs(guessScoreOfPreviousDecision - CurveGameProgress.TotalScoreDecision1);
                    //strategies[CurrentDecisionNumber.Value].AddScore(CurveGameProgress.TotalScoreDecision2);
                    break;
                default:
        ...
        protected override List<double> GetDecisionInputs()
        {
            ...
            {
                case 0:
                    // CurveFitting5DGame's Curves are in the format of a list of double inputs, followed by one double, the target output.
