    ...
    public class SimpleGameDefinition : GameDefinition, ICodeBasedSettingGenerator, ICodeBasedSettingGeneratorName
    {
        ...
        private static List<PlayerInfo> GetPlayersList()
        {
            ...
            return new List<PlayerInfo>
                {
                    new PlayerInfo(PlaintiffName, (int) SimpleGamePlayers.Plaintiff, false, true),
                    new PlayerInfo(DefendantName, (int) SimpleGamePlayers.Defendant, false, true),
                    new PlayerInfo(LitigationQualityChanceName, (int) SimpleGamePlayers.QualityChance, true, false),
                    new PlayerInfo(PlaintiffSignalChanceName, (int) SimpleGamePlayers.PSignalChance, true, false),
                    new PlayerInfo(DefendantSignalChanceName, (int) SimpleGamePlayers.DSignalChance, true, false),
                    new PlayerInfo(CourtChanceName, (int) SimpleGamePlayers.CourtChance, true, false),
                    new PlayerInfo(ResolutionPlayerName, (int) SimpleGamePlayers.Resolution, true, false),
                };
        }
        ...
        private List<Decision> GetDecisionsList()
        {
            ...
            List<byte> playersKnowingLitigationQuality = new List<byte>() { (byte) SimpleGamePlayers.PSignalChance, (byte)SimpleGamePlayers.DSignalChance, (byte) SimpleGamePlayers.CourtChance };
            if (PNoiseStdev == 0)
                playersKnowingLitigationQuality.Add((byte)SimpleGamePlayers.Plaintiff);
            if (DNoiseStdev == 0)
                playersKnowingLitigationQuality.Add((byte)SimpleGamePlayers.Defendant);
            decisions.Add(new Decision("LitigationQuality", "Qual", (byte)SimpleGamePlayers.QualityChance, playersKnowingLitigationQuality, NumLitigationQualityPoints, (byte)SimpleGameDecisions.LitigationQuality));
            // Plaintiff and defendant signals. If a player has perfect information, then no signal is needed.
            if (PNoiseStdev != 0)
                decisions.Add(new Decision("PlaintiffSignal", "PSig", (byte)SimpleGamePlayers.PSignalChance, new List<byte> { (byte)SimpleGamePlayers.Plaintiff }, NumSignals, (byte)SimpleGameDecisions.PSignal, unevenChanceActions: true));
            if (DNoiseStdev != 0)
                decisions.Add(new Decision("DefendantSignal", "DSig", (byte)SimpleGamePlayers.DSignalChance, new List<byte> { (byte)SimpleGamePlayers.Defendant }, NumSignals, (byte)SimpleGameDecisions.DSignal, unevenChanceActions: true));
            for (int b = 0; b < NumBargainingRounds; b++)
            {
                ...
                {
                    informationSetsToAddPlaintiffMoveTo.Add((byte)SimpleGamePlayers.Plaintiff);
                    informationSetsToAddDefendantMoveTo.Add((byte)SimpleGamePlayers.Defendant);
                }
                ...
                { // samuelson-chaterjee bargaining
                    ...
                    {
                        informationSetsToAddPlaintiffMoveTo.Add((byte)SimpleGamePlayers.Defendant);
                        informationSetsToAddDefendantMoveTo.Add((byte)SimpleGamePlayers.Plaintiff);
                    }
                    decisions.Add(new Decision("PlaintiffOffer" + (b + 1), "PO" + (b + 1), (byte)SimpleGamePlayers.Plaintiff, informationSetsToAddPlaintiffMoveTo, NumOffers, (byte)SimpleGameDecisions.POffer));
                    decisions.Add(new Decision("DefendantOffer" + (b + 1), "DO" + (b + 1), (byte)SimpleGamePlayers.Defendant, informationSetsToAddDefendantMoveTo, NumOffers, (byte)SimpleGameDecisions.DOffer) { CanTerminateGame = true });
                }
                else
                { // offer-response bargaining
                    ...
                    {
                        decisions.Add(new Decision("PlaintiffOffer" + (b + 1), "PO" + (b + 1), (byte)SimpleGamePlayers.Plaintiff, informationSetsToAddPlaintiffMoveTo, NumOffers, (byte)SimpleGameDecisions.POffer)); // { AlwaysDoAction = 4});
                        decisions.Add(new Decision("DefendantResponse" + (b + 1), "DR" + (b + 1), (byte)SimpleGamePlayers.Defendant, informationSetsToAddDefendantMoveTo, 2, (byte)SimpleGameDecisions.DResponse) { CanTerminateGame = true });
                    }
                    else
                    {
                        decisions.Add(new Decision("DefendantOffer" + (b + 1), "DO" + (b + 1), (byte)SimpleGamePlayers.Defendant, informationSetsToAddDefendantMoveTo, NumOffers, (byte)SimpleGameDecisions.DOffer));
                        decisions.Add(new Decision("PlaintiffResponse" + (b + 1), "PR" + (b + 1), (byte)SimpleGamePlayers.Plaintiff, informationSetsToAddPlaintiffMoveTo, 2, (byte)SimpleGameDecisions.PResponse) { CanTerminateGame = true });
                    }
                }
            }
            decisions.Add(new Decision("CourtDecision", "CD", (byte)SimpleGamePlayers.CourtChance, new List<byte> { (byte)SimpleGamePlayers.Resolution }, 2 /* for plaintiff or for defendant */, (byte)SimpleGameDecisions.CourtDecision, unevenChanceActions: true) { CanTerminateGame = true });
            return decisions;
        }
        ...
        public override void CustomInformationSetManipulation(Decision currentDecision, byte currentDecisionIndex, byte actionChosen, ref GameHistory gameHistory)
        {
            ...
            if (decisionByteCode >= (byte)SimpleGameDecisions.POffer && decisionByteCode <= (byte)SimpleGameDecisions.DResponse)
            {
                byte numItems = gameHistory.CountItemsInInformationSet((byte)SimpleGamePlayers.Resolution);
                if (numItems == 3)
                {
                    numItems = 0;
                    gameHistory.ReduceItemsInInformationSet((byte)SimpleGamePlayers.Resolution, decisionByteCode, numItems);
                }
                if (numItems == 0)
                    gameHistory.AddToInformationSet(decisionByteCode, currentDecisionIndex, (byte)SimpleGamePlayers.Resolution); 
                gameHistory.AddToInformationSet(actionChosen, decisionByteCode, (byte)SimpleGamePlayers.Resolution);
            }
        }
        ...
        public override bool ShouldMarkGameHistoryComplete(Decision currentDecision, GameHistory gameHistory)
        {
            if (!currentDecision.CanTerminateGame)
                return false;
            byte decisionByteCode = currentDecision.DecisionByteCode;
            if (decisionByteCode == (byte)SimpleGameDecisions.CourtDecision)
                return true;
            if (decisionByteCode == (byte)SimpleGameDecisions.DResponse || decisionByteCode == (byte)SimpleGameDecisions.PResponse)
            {
                var lastTwoActions = gameHistory.GetLastTwoActions();
                if (lastTwoActions.mostRecentAction == 1) // offer was accepted
                    return true;
            }
            else if (decisionByteCode == (byte)SimpleGameDecisions.DOffer)
            {
            ...
            else if (decisionByteCode == (byte)SimpleGameDecisions.POffer)
            {
                // this is simultaneous bargaining with plaintiff offer first. 
