...
namespace ACESim
{
    ...
    public static class FieldwiseObjectComparison
    {
        ...
        private static bool AreEqualHelper2(object obj1, object obj2, bool throwIfUnequal, bool provideReport, List<string> fieldNamesList, string preText, ref string textToReport, int level, List<object> objectsAlreadyProcessed)
        {
            ...
            if (objectsAlreadyProcessed.Any(x => Object.ReferenceEquals(x, obj1) && Object.ReferenceEquals(x, obj2)))
            ...
            if (obj1.GetType() == obj2.GetType())
            {
                ...
                foreach (FieldInfo field in fields)
                {
                    ...
                    //    }
                    //}
                    else if (fieldVal1 is IEnumerable)
                    {
                        ...
                        else
                        {
                            ...
                            while (returnVal && array1Enum.MoveNext() && array2Enum.MoveNext())
                            {
                                ...
                                { // Neither is null
                                    List<string> deeperFieldNamesList2 = deeperFieldNamesList.Concat(new List<string> { array1Enum.Current.GetType().ToString() }).ToList();
                                    returnVal = AreEqualHelper2(array1Enum.Current, array2Enum.Current, throwIfUnequal, provideReport, deeperFieldNamesList2, fieldName + "[" + index + "]" + ": ", ref textToAddToReport, level + 1, objectsAlreadyProcessed);
                                }
                                index++;
                            }
                            if (returnVal == false)
