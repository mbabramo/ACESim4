...
namespace PriorityQueue
{
    ...
    [Serializable]
    public class PriorityQueue<TPriority, TValue> : ICollection<KeyValuePair<TPriority, TValue>> where TPriority : struct
    {
        int? MaxCapacity;
        private List<KeyValuePair<TPriority, TValue>> _baseHeap;
        private IComparer<TPriority> _comparer;
        public TPriority? LowestPriorityOrNullIfNotYetFull;
        public TPriority LowestPriorityWhetherOrNotFull;
        public object lockObjEnqueue = new object();
        public object lockObjDequeue = new object();
        public object lockObjGetValues = new object();
        ...
        public PriorityQueue(int initialCapacity, int maxCapacity)
            : this(initialCapacity, maxCapacity, Comparer<TPriority>.Default)
        {
        }
        ...
        /// </summary>
        public PriorityQueue(int initialCapacity, int maxCapacity, IComparer<TPriority> comparer)
        {
            if (comparer == null)
                throw new ArgumentNullException();

            _baseHeap = new List<KeyValuePair<TPriority, TValue>>(Math.Min(initialCapacity, 1000));
            _comparer = comparer;
        ...
        public PriorityQueue<TPriority, TValue> DeepCopy()
        {
            PriorityQueue<TPriority, TValue> pq = new PriorityQueue<TPriority, TValue>(_baseHeap, _comparer);
        ...
        public static PriorityQueue<TPriority, TValue> MergeQueues(PriorityQueue<TPriority, TValue> pq1, PriorityQueue<TPriority, TValue> pq2, IComparer<TPriority> comparer)
        {
            if (pq1 == null || pq2 == null || comparer == null)
                throw new ArgumentNullException();
            // merge data
            PriorityQueue<TPriority, TValue> result = new PriorityQueue<TPriority, TValue>((int) (pq1.Count + pq2.Count), (int) (pq1.MaxCapacity + pq2.MaxCapacity), pq1._comparer);
        ...
        public TPriority? Enqueue(TPriority priority, TValue value, out bool full)
        {
            lock (lockObjEnqueue)
            {
                if (MaxCapacity == _baseHeap.Count)
                {
                    full = true;
                    TPriority nextDequeue = _baseHeap[0].Key;
                    if (_comparer.Compare(priority, nextDequeue) == 1)
                    {
                        Dequeue();
                        Insert(priority, value);
                    }
                    LowestPriorityOrNullIfNotYetFull = LowestPriorityWhetherOrNotFull = (TPriority)_baseHeap[0].Key;
                    return _baseHeap[0].Key;
                }
                else
                {
                    Insert(priority, value);
                    full = false;
                    LowestPriorityWhetherOrNotFull = (TPriority)_baseHeap[0].Key;
        ...
        {
            lock (lockObjGetValues)
            {
                List<TValue> li = new List<TValue>();
