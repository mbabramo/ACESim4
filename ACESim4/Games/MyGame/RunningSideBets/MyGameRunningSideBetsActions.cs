...
namespace ACESim
{
    ...
    {
        public List<(byte PAction, byte DAction)> ActionsEachBargainingRound;
        ...
        public byte GetTotalChipsThatCount(byte? roundOfAbandonment, bool playerAbandoningIsPlaintiff)
        {
            ...
            if (ActionsEachBargainingRound != null)
                chipsPreviousRounds = (byte) ActionsEachBargainingRound.Take(roundOfAbandonment == null ? int.MaxValue : (byte) (roundOfAbandonment - 1)).Sum(x => Math.Max(x.PAction, x.DAction));
            ...
            if (roundOfAbandonment != null)
            {
                ...
                if (playerAbandoningIsPlaintiff)
                    chipsAbandoningPlayerAgreedTo = chipsBetLastRound.PAction;
                else
                    chipsAbandoningPlayerAgreedTo = chipsBetLastRound.DAction;
            }
