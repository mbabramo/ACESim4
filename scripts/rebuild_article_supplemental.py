...
The [full strategy analysis](Tables/manuscript-strategy-mechanisms.pdf) retains every qualifying row for seven ordinary-cost comparisons, including sensitivity diagnostics. The [short manuscript table](Tables/selected-strategy-mechanisms.pdf) selects three risk-neutral rows: the demand change under Trial fees, and answering and filing changes under Complete fees. It omits sensitivity markers and the opponent-exit and remaining columns, which are zero in these rows. The [risk-averse manuscript table](Tables/selected-risk-averse-strategy-mechanisms.pdf) adds eleven illustrative rows, including American-rule demand and offer changes and Trial-rule defendant exit commitments, and retains opponent-exit contributions. Both manuscript selections omit zero remaining contributions. The full data and diagnostics remain available here. Captions are separate files in Sources; the shared methodology explains both displays.
...
The ACESim4 code repository's scripts/Rebuild-ArticleSupplemental.ps1 generates the separate analyses; scripts/Article-supplemental.md documents its options. Python prepares requests and this index; C# calculates the comparisons, selects table rows, and generates TeX and JSON. LitigCharts equilibrium-manuscript assembles the complete seven-panel analysis and the short manuscript selection; --article refreshes the numbered Table 2 with the short selection.
...
def prepare(results,output,exe,log_roots):
    ...
    manuscript_config=CODE/'LitigCharts/ArticleStrategyComparisons.json'
    manuscript_ids=[p['Id']+'-cost-1' for p in read(manuscript_config)]
    available={j['id'] for j in jobs}
    dependencies=['table-'+id for id in manuscript_ids]
    if not set(dependencies)<=available:
        raise ValueError('Missing a required ordinary-cost manuscript comparison')
    manuscript_inputs=[manuscript_config]
    for id in manuscript_ids:
        directory=changes/'Data/cost-1'/id
        manuscript_inputs.extend([directory/'equilibrium-changes.request.json',
            directory/'equilibrium-changes-manifest.json',directory/(id+'.json')])
    stems=['manuscript-strategy-mechanisms','selected-strategy-mechanisms','selected-risk-averse-strategy-mechanisms']
    job('table-manuscript','tables',[exe,'equilibrium-manuscript','--input',changes],
        dependencies,manuscript_inputs,
        [path for stem in stems for path in
         [changes/'Tables'/(stem+'.pdf'),changes/'Tables'/(stem+'.png'),
          changes/'Sources/Tex'/(stem+'.tex'),changes/'Sources/Json'/(stem+'.json'),
          changes/'Sources'/(stem+'-caption.txt')]],
        [pattern for stem in stems for pattern in
         [(str(changes/'Tables'),stem+'*'),(str(changes/'Sources/Tex'),stem+'.tex'),
          (str(changes/'Sources/Json'),stem+'.json'),(str(changes/'Sources'),stem+'-caption.txt')]])
