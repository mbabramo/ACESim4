...
`LitigCharts equilibrium-manuscript --input <Equilibrium strategy changes> [--article <article repository>]` retains every qualifying coordinate for the seven ordered comparisons in `LitigCharts/ArticleStrategyComparisons.json` as `manuscript-strategy-mechanisms`. It also generates `selected-strategy-mechanisms`, the three-row risk-neutral manuscript selection. It also generates the eleven-row risk-averse selection, `selected-risk-averse-strategy-mechanisms`. The optional article argument publishes these selections as Tables 2 and 3. The full packet preserves all sensitivity diagnostics and residuals; the short table omits sensitivity markers and columns that are zero in every selected row. Saved calculations are reused, without re-solving. The supplemental scheduler includes all three displays after the seven source tables.
