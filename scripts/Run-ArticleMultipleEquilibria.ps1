param(
    ...
    [int]$Processors = [Math]::Min(6, [Environment]::ProcessorCount),
    [ValidateSet('multiple-equilibria','agreement-to-bargain')][string]$Plan = 'multiple-equilibria',
    [switch]$SkipBuild,
    [switch]$SolveOnly
)
...
$state = [ordered]@{Status='Building';StartedUtc=[datetime]::UtcNow.ToString('o');
    Source=$repo;Output=$output;Processors=$Processors;Plan=$Plan;Pid=$PID}
...
try {
    ...
    $state | ConvertTo-Json | Set-Content -LiteralPath $stateFile -Encoding utf8
    & $runner run --plan $Plan --processors $Processors --hidden-workers --results-directory $production
    if ($LASTEXITCODE -ne 0) { throw 'Multiple-equilibrium solving failed; inspect production records before resuming.' }
    $state.Status='Aggregating'
    $state | ConvertTo-Json | Set-Content -LiteralPath $stateFile -Encoding utf8
    & $runner aggregate --plan $Plan --results-directory $production
    if ($LASTEXITCODE -ne 0) { throw 'Multiple-equilibrium aggregation failed.' }
    if ($SolveOnly) { $state.Status='SolvedAndAggregated'; return }
    $state.Status='Rendering'
    $state | ConvertTo-Json | Set-Content -LiteralPath $stateFile -Encoding utf8
    & (Join-Path $repo 'LitigCharts\bin\Release\net9.0\LitigCharts.exe') multiple-equilibria-report --plan $Plan --input $production --output $output --jobs $Processors
