    ...
    public class PriorityQueue<TPriority, TValue> : ICollection<KeyValuePair<TPriority, TValue>> where TPriority : struct
    {
        ...
        public static PriorityQueue<TPriority, TValue> MergeQueues(PriorityQueue<TPriority, TValue> pq1, PriorityQueue<TPriority, TValue> pq2, IComparer<TPriority> comparer)
        {
            if (pq1 == null || pq2 == null || comparer == null)
                throw new ArgumentNullException();
            // merge data
            PriorityQueue<TPriority, TValue> result = new PriorityQueue<TPriority, TValue>(pq1.Count + pq2.Count, (int)(pq1.MaxCapacity + pq2.MaxCapacity), pq1._comparer);
        ...
        public TPriority? Enqueue(TPriority priority, TValue value, out bool full)
        {
            lock (lockObjEnqueue)
            {
                if (MaxCapacity == _baseHeap.Count)
                {
                    ...
                    LowestPriorityOrNullIfNotYetFull = LowestPriorityWhetherOrNotFull = _baseHeap[0].Key;
                ...
                else
                {
                    Insert(priority, value);
                    full = false;
                    LowestPriorityWhetherOrNotFull = _baseHeap[0].Key;
