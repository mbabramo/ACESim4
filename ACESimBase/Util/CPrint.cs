...
namespace ACESimBase.Util
{
	public static class CPrint
	{
		...
		public static string getstring(char[] buf, int posinbuf)
		{
			int terminatorIndex = posinbuf;
			while (buf[terminatorIndex] != '\0')
				terminatorIndex++;
			StringBuilder b = new StringBuilder();
			for (int i = posinbuf; i < terminatorIndex; i++)
				b.Append(buf[i]);
			return b.ToString();
		}

		public static void strcpy(char[] buf, int posinbuf, string s)
		{
		...
		internal static int strcpy_formatted(char[] buf, int sindex, string Format, params object[] Parameters)
		{
			string s = sprintf(Format, Parameters);
			strcpy(buf, sindex, s);
