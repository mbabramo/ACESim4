...
namespace ACESimBase.Util.Serialization
{
    ...
    public static class FarmhashByteSpans
    {
        ...
        private static uint Hash32Len5Plus(ReadOnlySpan<byte> s, uint len)
        {
            ...
            do
            {
                uint a = Fetch(s.Slice(0 + s_index));
                uint b = Fetch(s.Slice(4 + s_index));
                uint c = Fetch(s.Slice(8 + s_index));
                uint d = Fetch(s.Slice(12 + s_index));
                uint e = Fetch(s.Slice(16 + s_index));
                h += a;
                g += b;
                f += c;
                h = Mur(d, h) + e;
        ...
        private static ulong HashLen65to96(ReadOnlySpan<byte> s, uint len)
        {
            ...
            ulong h0 = H32(s, 32, mul0);
            ulong h1 = H32(s.Slice(32), 32, mul1);
        ...
        private static void ConvertIntToShorts(int value, out short valueHigh, out short valueLow)
        {
            ...
            valueHigh = (short)(value >> 16);
            valueLow = (short)(value & LOW_MASK);
        }
