...
namespace ACESimBase.Util.Parallelization
{
    public static class Parallelizer
    {
        ...
        public static int? MaxDegreeOfParallelism = null;
        ...
        {
            Go(doParallel, start, (long)stopBeforeThis, x => { action((int)x); }, cancellationToken);
        ...
        {
            GoForSpecifiedNumberOfSuccesses(doParallel, numSuccessesRequired, (x, y) => actionReturningTrueUponSuccess((int)x, (int)y), (long)firstIterationNumberToTry, minSuccessRate);
            //if (ParallelDepth > 1)
            //    doParallel = false;
        ...
        private static void DoUntilSuccessful(Func<int, int, bool> actionReturningTrueUponSuccess, int successNumber, ref int nextIterationNumberToTry, ref int successCount, ref int failureCount, int? maxNumberAttempts = null, int? returnIfNoSuccessAfterAttempts = null)
        {
            ...
            DoUntilSuccessful((x, y) => actionReturningTrueUponSuccess((int)x, (int)y), successNumber, ref nextIter);
                        ...
                        Task childTask = Task.Factory.StartNew(() =>
                            {
                                ...
                                lock (lockObj)
                                {
                                    ...
                                    { // checkpoint
                                        if (numSuccessesSoFar / (double)numCompleted < minSuccessRate)
                                            abort = true;
                                        else
