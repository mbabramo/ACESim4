...
namespace ACESim.Util
{
    ...
    public static class DiscreteValueSignalOld // DEBUG
    {
        /// <summary>
        ...
        /// For example, if NumSignals is 10, then one-tenth of the time, we will draw from the two distributions in a way that will produce a signal of i, for every i from 1 to 10. 
        /// </summary>
        ...
        /// <summary>
        /// Given a draw of a band from a uniform distribution, returns the probability that a signal consisting of the sum of the midpoint of this band and a value drawn from a normal distribution would end up in each discrete band of signals, where each signal band is of equal size.
        /// </summary>
        /// <param name="actualUniformDistributionValue">The discrete band of the uniform distribution (numbered 1 .. number of bands)</param>
        /// <param name="discreteSignal">The discrete band of the signal (numbered 1 .. number of signals)</param>
        /// <param name="dsParams">The parameters specifying the noise and the number of signals and uniform distribution points</param>
        ...
        private static void CalculateCutoffs(DiscreteValueSignalParameters nsParams)
        {
            ...
            var equallySpacedPoints = EquallySpaced.GetEquallySpacedPoints(nsParams.NumPointsInSourceUniformDistribution, nsParams.SourcePointsIncludeExtremes);
            double[] sourcePoints = equallySpacedPoints;

            double stdev = nsParams.StdevOfNormalDistribution;
            if (stdev == 0)
                stdev = 1E-10;
            // Now we want the cutoffs for the signals. We need this whether we make all signals equally likely or not. 
            ...
            {
                // Make each signal equally likely. This makes sense if this is a private signal of quality.
                // Note that if we want 2 signals, then we want 1 cutoff at 0.5(i.e., 50th percentiles); if there are 10 signals, we want cutoffs at percentiles corresponding to .1, .2, ..., .9.
            ...
            {
                // Some signals may be more likely than others. This makes sense if this is a quality measurement based on some underlying true value (e.g., litigation quality based on truly liable or truly not liable).                  
                // Get the cross product of the normal distribution for cutoff points and the source points, then take an ordered list of these sums. Thus, the higher the standard deviation we're using for this, the further apart the signal value cutoffs will be (and, if the normal distribution that we are using has lower variance, the less likely it will be that we hit extreme values). If the number of signals is even, then the number of cutoffs will be odd, and the middle cutoff will be 0.5, assuming that the source points are 0.0 and 1.0
        ...
        private static double ValueAtPercentile(double[] orderedSequence, double percentile)
        {
