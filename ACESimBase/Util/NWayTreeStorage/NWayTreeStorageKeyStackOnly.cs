...
using ACESim.Util;
using static ACESimBase.Util.Collections.ListExtensions;
...
namespace ACESimBase.Util.NWayTreeStorage
{
    public ref struct NWayTreeStorageKeyStackOnly
    {
        ...
        public override string ToString() => PrefaceByte + ": " + string.Join(",", GetSpan255TerminatedAsList(Sequence));

        public NWayTreeStorageKey ToStorable()
        {
            return new NWayTreeStorageKey(PrefaceByte, GetSpan255TerminatedAsList(Sequence).ToArray());
        }
