...
namespace ACESimBase.GameSolvingSupport
{
    public partial class GameProgressTree : IEnumerable<GameProgress>, IDisposable
    {
        ...
        public static (IDirectGamePlayer gamePlayer, int numObservations)[][] GetDirectGamePlayersForEachDecision(GameProgressTree[] gameProgressTrees, double explorationValue, int[] targetObservations, bool oversampling)
        {
            ...
            for (byte playerIndex = 0; playerIndex < gameProgressTrees.Count(); playerIndex++)
            {
                ...
                var resultForPlayer = gameProgressTree.GetDirectGamePlayersForEachDecision(explorationValues, targetObservations, oversampling);
                if (playerIndex == 0)
        ...
        public (IDirectGamePlayer gamePlayer, int numObservations)[][] GetDirectGamePlayersForEachDecision(double[] explorationValues, int[] targetObservations, bool oversampling)
        {
            ...
            for (byte decisionIndex = 0; decisionIndex < NumDecisionIndices; decisionIndex++)
            {
                if (DecisionsList[decisionIndex].IsChance || (LimitToPlayer != null && DecisionsList[decisionIndex].PlayerIndex != LimitToPlayer) || (targetObservations[decisionIndex] == 0 && oversampling))
                    continue;
                var directGamePlayersWithCounts = GetDirectGamePlayersForDecisionIndex(explorationValues, decisionIndex, oversampling).ToList();
                ...
                if (adjustedTarget == totalAvailableCount || !oversampling)
        ...
        public IEnumerable<(IDirectGamePlayer gamePlayer, int numObservations)> GetDirectGamePlayersForDecisionIndex(double[] explorationValues, byte decisionIndex, bool oversampling)
        {
            ...
            if (oversampling)
            {
                ...
                allocationIndex = AllocationIndexForExplorationProbabilityAndDecisionIndex[key];
            }
            IEnumerable<NWayTreeStorage<GameProgressTreeNodeInfo>> nodesStorage = GetNodesForAllocationIndex(explorationValues, allocationIndex, decisionIndex).ToList();
