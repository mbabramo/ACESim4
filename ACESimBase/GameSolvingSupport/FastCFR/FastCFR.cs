...
namespace ACESimBase.GameSolvingSupport.FastCFR
{
        ...
        where TScalar : struct
    {
        ...
        private FastCFRCall[][] _childCallsByVisit; // avoids interface dispatch
        ...
        // Persistent scratch
        private readonly double[] _qaBuffer;     // Qa per infoset (double)
        private readonly float[] _chunkF;        // Vector<float>.Count scratch to avoid stackalloc
        ...
        protected FastCFRInformationSetGeneric(byte playerIndex, byte decisionIndex, byte numActions, FastCFRVisitProgram[] visits)
        {
            ...
            _qaBuffer = new double[numActions];
        ...
        internal override void BindChildrenAfterFinalize()
        {
            ...
            _childCallsByVisit = new FastCFRCall[_visits.Length][];
            for (int v = 0; v < _visits.Length; v++)
            {
                var steps = _visits[v].Steps;
                var nodes = new IFastCFRNode[steps.Length];
                var calls = new FastCFRCall[steps.Length];
                for (int i = 0; i < steps.Length; i++)
                {
                    var n = steps[i].ChildAccessor();
                    nodes[i] = n;
                    calls[i] = n.Go; // instance-open delegate, allocated once
                }
                _childrenByVisit[v] = nodes;
                _childCallsByVisit[v] = calls;
            }
        }
        ...
        public override FastCFRNodeResult Go(ref FastCFRIterationContext ctx)
        {
            ...
            var calls = _childCallsByVisit[visitIndex];
            ...
            Span<double> Qa = ownerIsOptimized ? _qaBuffer.AsSpan(0, NumActions) : Span<double>.Empty;
            ...
            for (int i = 0; i < children.Length; i++)
            {
                ...
                // Opponent side: skip math if traversal probability ~ 0
            ...
            if (ownerIsOptimized)
            {
                ...
                // One scalar add per visit (denominator is action-invariant)
                _sumInversePiShared += inversePi;
            }
        ...
        private double DotProductFloatChunked(ReadOnlySpan<double> QaD)
        {
            ...
            while (i <= n - vc)
            {
                for (int k = 0; k < vc; k++) _chunkF[k] = (float)QaD[i + k];
        ...
        private void SimdUpdateFloatChunked(ReadOnlySpan<double> QaD, float V, float inversePi, float piSelf)
        {
            ...
            while (i <= n - vc)
            {
                ...
                var vSumR = new Vector<float>(sumR.Slice(i, vc)) + ((vQa - vV) * vInv);
                vSumR.CopyTo(sumR.Slice(i, vc));

                var vLast = new Vector<float>(last.Slice(i, vc)) + (vP * vPiS);
                vLast.CopyTo(last.Slice(i, vc));
    ...
    // ----------------------------------------------------------------------
    // Chance node (caches per-iteration probabilities when dynamic)
    // ----------------------------------------------------------------------
    public sealed class FastCFRChance : IFastCFRNode
    {
        ...
        private double[][] _cachedProbs;   // recomputed each iteration for dynamic visits
        ...
        internal void BindChildrenAfterFinalize()
        {
            ...
            for (int v = 0; v < _visits.Length; v++)
            {
                ...
                {
                    _cachedProbs[v] = new double[steps.Length]; // allocated once, filled per iteration
                }
            }
        }
        ...
        {
            // Precompute dynamic probabilities once per iteration (path-independent)
            for (int v = 0; v < _visits.Length; v++)
            {
                if (_visitHasDynamic[v])
                {
                    var steps = _visits[v].Steps;
                    var dest = _cachedProbs[v];
                    var dummy = new FastCFRIterationContext(); // provider in builder does not depend on ctx
                    for (int i = 0; i < steps.Length; i++)
                        dest[i] = steps[i].GetProbability(ref dummy);
                }
            }
            _visitCounter = 0;
        }
        ...
        public FastCFRNodeResult Go(ref FastCFRIterationContext ctx)
        {
            ...
            var visit = _visits[visitIndex];
            var children = _childrenByVisit[visitIndex];
            var probs = _visitHasDynamic[visitIndex] ? _cachedProbs[visitIndex] : _staticProbs[visitIndex];
