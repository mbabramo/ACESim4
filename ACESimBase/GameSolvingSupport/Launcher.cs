...
namespace ACESim
{
    public abstract class Launcher
    {
        ...
        public virtual string MasterReportNameForDistributedProcessing => "R370"; // ******* IMPORTANT: (1) Delete bin/obj in AceSimDistributed. Make sure that the correct Launcher is set in ACESimDistributed Program.cs. (2) Must update this (or delete the Coordinator) when deploying service fabric. 
        ...
        public async Task ParticipateInDistributedProcessing(string masterReportName, CancellationToken cancellationToken, Action<string> logAction = null)
        {
            ...
            while (!complete)
            {
                ...
                AzureBlob.TransformSharedBlobOrFileObject(ReportFolder(), "results", masterReportName + " Coordinator", o =>
                {
                    ...
                    if (taskToDo != null)
                    {
                        int? alwaysDoTaskID = 76; // DEBUG null; // set this to a task to replay a particular task over and over
