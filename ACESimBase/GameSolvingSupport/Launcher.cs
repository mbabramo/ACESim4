...
namespace ACESim
{
    public abstract class Launcher
    {
        ...
        public IStrategiesDeveloper GetInitializedDeveloper(GameOptions options)
        {
            ...
            var developer = GetStrategiesDeveloper();
            developer.Strategies = starterStrategies;
            developer.EvolutionSettings = evolutionSettings;
            developer.GameDefinition = gameDefinition;
            return developer;
        }

        public IStrategiesDeveloper GetStrategiesDeveloper()
        {
            switch (Algorithm)
            {
                case GameApproximationAlgorithm.Vanilla:
                    return new VanillaCFR();
                case GameApproximationAlgorithm.GibsonProbing:
                    return new GibsonProbing();
                case GameApproximationAlgorithm.ModifiedGibsonProbing:
                    return new ModifiedGibsonProbing();
                case GameApproximationAlgorithm.ExploratoryProbing:
                    return new ExploratoryProbing();
                case GameApproximationAlgorithm.HedgeProbing:
                    return new HedgeProbing();
                case GameApproximationAlgorithm.HedgeVanilla:
                    return new HedgeVanilla();
                case GameApproximationAlgorithm.AverageStrategySampling:
                    return new AverageStrategiesSampling();
                case GameApproximationAlgorithm.PureStrategyFinder:
                    return new PureStrategiesFinder();
                case GameApproximationAlgorithm.FictitiousSelfPlay:
                    return new FictitiousSelfPlay();
                default:
