...
namespace ACESimBase.GameSolvingSupport.GameTree
{
    ...
    public class NodeActionsHistory
    {
        ...
        public NodeActionsHistory WithPrepended(IGameState node, byte actionAtNode)
        {
            if (node == null)
                throw new Exception();
            ...
            if (!prepended.ModifyCoefficient(node))
                copied.Insert(0, new NodeAction(node, actionAtNode));
            return prepended;
        }

        public NodeActionsHistory WithAppended(IGameState node, byte actionAtNode)
        {
            if (node == null)
                throw new Exception();
            var copied = NodeActions.ToList();
            ...
            if (!appended.ModifyCoefficient(node))
                copied.Add(new NodeAction(node, actionAtNode));
            return appended;
        }
        ...
        /// <summary>
        /// Returns a list of actions, excluding a specified player. The decisions included are thus all those that may differentiate the action taken at the current information set.
        /// </summary>
        /// <param name="excludePlayerIndex"></param>
        /// 
        /// <returns></returns>
        public ByteList GetActionsListExcludingPlayer(byte? excludePlayerIndex)
        {
        ...
        public double GetProbabilityOfPath(bool useCurrentStrategyForProbability)
        {
            ...
            foreach (var nodeAction in NodeActions)
            {
                ...
                {
                    case ChanceNode c:
                        pathProbability *= c.GetActionProbability(nodeAction.ActionAtNode);
                        break;
        ...
        public (double probabilityOfPath, double probabilitySinceOpponentInformationSet, InformationSetNode mostRecentOpponentInformationSet, byte actionAtOpponentInformationSet) GetProbabilityOfPathPlus()
        {
            ...
            foreach (var nodeAction in NodeActions)
            {
                ...
                {
                    case ChanceNode c:
                        double chanceProbability = c.GetActionProbability(nodeAction.ActionAtNode);
