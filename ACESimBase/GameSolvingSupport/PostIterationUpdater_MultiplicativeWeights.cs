...
namespace ACESimBase.GameSolvingSupport
{
    public class PostIterationUpdater_MultiplicativeWeights : PostIterationUpdaterBase
    {
        ...
        public override void UpdateInformationSet(InformationSetNode node)
        {
            ...
            {
                double denominator = nodeInformation[InformationSetNode.lastRegretDenominatorDimension, a - 1];
                double regretUnnormalized = (denominator == 0) ? 0.5 * (node.MaxPossibleThisPlayer - node.MinPossibleThisPlayer) : nodeInformation[InformationSetNode.lastRegretNumeratorDimension, a - 1] / denominator;
                double regret = node.NormalizeRegret(regretUnnormalized); // bad moves are now close to 0 and good moves are close to 1
                ...
                sumWeights += weight;
                nodeInformation[InformationSetNode.lastRegretNumeratorDimension, a - 1] = 0; // reset for next iteration
                nodeInformation[InformationSetNode.lastRegretDenominatorDimension, a - 1] = 0;
            }
            if (sumWeights < 1E-20)
