...
namespace ACESimBase.GameSolvingAlgorithms.ECTAAlgorithm
{
    public class ECTATreeDefinition
    {
        ...
        public void normalizeMaxPayoutToNegative1(bool bprint)
        {
            ...
            {
                maxpay[playerIndex] = FromInteger(MINUSINFTY);
                ...
                addtopay[playerIndex] = Negate(Add(maxpay[playerIndex], FromInteger(1)));
                for (int zindex = 0; zindex < outcomes.Length; zindex++)
                {
                    z = outcomes[zindex];
                    z.pay[playerIndex] = Add(z.pay[playerIndex], addtopay[playerIndex]);
                }
            }
        }
        ...
        public void outputGameTree()
        {
            ...
            for (int uindex = 1; uindex < nodes.Length; uindex++)
            {
                ...
                for (pl = 1; pl < PLAYERS; pl++)
                    if (u.terminal)
                    {
                        s = ToNumericString(outcomes[u.outcome].pay[pl - 1]);
                        colpr(s);
                    }
                    else
                        colpr("");
            ...
            for (int cindex = 0; cindex < moves.Length; cindex++)
            {
                ...
                s = ToNumericString(c.behavioralProbability); 
                colpr(s);
                s = ToNumericString(c.realizationProbability); 
                colpr(s);
                colipr(c.redsfcol);
                colipr(c.ncompat);
                colipr(c.offset);
        ...
        public void gencentroid()
        {
            ...
            for (pl = 1; pl < ECTATreeDefinition.PLAYERS; pl++)
                ...
                {
                    c = moves[cindex];
                    c.behavioralProbability = (Rational) 1 / (Rational) informationSets[c.priorInformationSet].numMoves;
                }
        }
            ...
            for (pl = 1; pl < PLAYERS; pl++)
                for (int hindex = firstInformationSet[pl]; hindex < firstInformationSet[pl + 1]; hindex++)
                {
                    ...
                    for (int i = 0; i < h.numMoves; i++)
                    {
                        ...
                        moves[h.firstMoveIndex + i].behavioralProbability = (Rational)numerator; // store value so that we remember it
                        denominator += numerator;
                    }
                    for (int i = 0; i < h.numMoves; i++)
                    {
                        Rational a = new Rational();
                        a = moves[h.firstMoveIndex + i].behavioralProbability.Numerator / (Rational)denominator;
            ...
            for (player1SequenceIndex = 0; player1SequenceIndex < numSequences[1]; player1SequenceIndex++)
                for (player2SequenceIndex = 0; player2SequenceIndex < numSequences[2]; player2SequenceIndex++)
                {
                    ...
                    for (playerIndex = 1; playerIndex < PLAYERS; playerIndex++) // skip chance palyer
                        sequenceFormPayouts[player1SequenceIndex][player2SequenceIndex][playerIndex - 1] = FromInteger(0);
                }
        ...
        void generateSequenceFormPayoutsAndConstraints()
        {
            ...
            for (int zindex = 0; zindex < outcomes.Length; zindex++)
            {
                ...
                for (pl = 1; pl < PLAYERS; pl++)
                    sequenceFormPayouts[player1SequenceIndex][player2SequenceIndex][pl - 1] = Add(sequenceFormPayouts[player1SequenceIndex][player2SequenceIndex][pl - 1],
        ...
        public void generateSequenceFormLCP()
        {
            ...
            */
            ...
            Lemke.rhsq[numSequences[1] + numInfoSets[2] + 1 + numSequences[2]] = Negate(FromInteger(1));
        }
        ...
        void outrealplan(int pl, Rational[] rplan, int offset)
        {
            ...
            for (i = 0; i < numSequences[pl]; i++)
            {
                s = ToNumericString(rplan[i + offset]);
                colpr(s);
            }
            colout();
        }
            ...
            for (int hindex = firstInformationSet[pl]; hindex < firstInformationSet[pl + 1]; hindex++)
            {
                ...
                for (int cindex = h.firstMoveIndex; i < h.numMoves; cindex++, i++)
                {
                    ...
                    if (rprob.Numerator != 0)
                    {
                        ...
                        if (!Equality(bprob, FromInteger(1)))
                        {
                            s = ToNumericString(bprob);
                            tabbedtextf(":%s", s);
                        }
                    }
                }
        ...
        private void MakePlayerMovesStrictlyMixed(int pl, Rational[] allMoveProbabilities, int offset, Rational minValue)
        {
            ...
            for (int hindex = firstInformationSet[pl]; hindex < firstInformationSet[pl + 1]; hindex++)
            {
                ...
                for (int cindex = h.firstMoveIndex; moveIndexInInformationSet < h.numMoves; cindex++, moveIndexInInformationSet++)
                {
                    ...
                    if (c.behavioralProbability < minValue)
        ...
        void sfprint()
        {
            ...
            for (i = 0; i < numSequences[1]; i++)
            {
                ...
                for (j = 0; j < numSequences[2]; j++)
                {
                    s = ToNumericString(sequenceFormPayouts[i][j][0]); 
                    colpr(s);
                }
                colnl();
                /* payoffs player 2 */
                ...
                for (j = 0; j < numSequences[2]; j++)
                {
                    s = ToNumericString(sequenceFormPayouts[i][j][1]); 
                    colpr(s);
                }
        ...
        public Rational[][] realizationPlan = new Rational[PLAYERS][];
        ...
        public void SetRealizationProbabilitiesFromBehavioralProbabilities(int pl)
        {
            ...
            moves[firstMove[pl]].realizationProbability = FromInteger(1);  /* empty seq has probability 1  */
            ...
            for (i = 0; i < numRowsInSourceMatrix; i++)
                for (j = 0; j < numColsInSourceMatrix; j++)
                    if (transpose)
                        ...
                        = negate ? Negate(sourceMatrix[i][j][playerIndexMinus1]) : sourceMatrix[i][j][playerIndexMinus1];
                    else
    ...
    { 1000, 0, 875, 688, 500, 703, 641, 578, 500, 312, 703, 641, 578, 703, 641, 578, 125, 1000, 0, 875, 688, 500, 633, 570, 508, 500, 312, 633, 570, 508, 633, 570, 508, 125, 1000, 0, 875, 688, 500, 562, 500, 188, 500, 312, 562, 500, 188, 562, 500, 188, 125, 1000, 0, 875, 688, 500, 633, 570, 508, 500, 312, 633, 570, 508, 633, 570, 508, 125, 1000, 0, 875, 688, 500, 562, 500, 188, 500, 312, 562, 500, 188, 562, 500, 188, 125, 1000, 0, 875, 688, 500, 242, 180, 117, 500, 312, 242, 180, 117, 242, 180, 117, 125, 1000, 0, 875, 688, 500, 562, 500, 188, 500, 312, 562, 500, 188, 562, 500, 188, 125, 1000, 0, 875, 688, 500, 242, 180, 117, 500, 312, 242, 180, 117, 242, 180, 117, 125, 1000, 0, 875, 688, 500, 172, 109, 47, 500, 312, 172, 109, 47, 172, 109, 47, 125 }
            ...
            z.nodeIndex = 86;
            z.pay[0] = FromInteger(pay[0][0]);
            z.pay[1] = FromInteger(pay[1][0]);
            z = outcomes[++zindex];
            nodes[87].father = 4;
            ...
            z.nodeIndex = 87;
            z.pay[0] = FromInteger(pay[0][1]);
            z.pay[1] = FromInteger(pay[1][1]);
            z = outcomes[++zindex];
            nodes[88].father = 6;
            ...
            z.nodeIndex = 88;
            z.pay[0] = FromInteger(pay[0][2]);
            z.pay[1] = FromInteger(pay[1][2]);
            z = outcomes[++zindex];
            nodes[89].father = 6;
            ...
            z.nodeIndex = 89;
            z.pay[0] = FromInteger(pay[0][3]);
            z.pay[1] = FromInteger(pay[1][3]);
            z = outcomes[++zindex];
            nodes[90].father = 6;
            ...
            z.nodeIndex = 90;
            z.pay[0] = FromInteger(pay[0][4]);
            z.pay[1] = FromInteger(pay[1][4]);
            z = outcomes[++zindex];
            nodes[91].father = 8;
            ...
            z.nodeIndex = 91;
            z.pay[0] = FromInteger(pay[0][5]);
            z.pay[1] = FromInteger(pay[1][5]);
            z = outcomes[++zindex];
            nodes[92].father = 8;
            ...
            z.nodeIndex = 92;
            z.pay[0] = FromInteger(pay[0][6]);
            z.pay[1] = FromInteger(pay[1][6]);
            z = outcomes[++zindex];
            nodes[93].father = 8;
            ...
            z.nodeIndex = 93;
            z.pay[0] = FromInteger(pay[0][7]);
            z.pay[1] = FromInteger(pay[1][7]);
            z = outcomes[++zindex];
            nodes[94].father = 7;
            ...
            z.nodeIndex = 94;
            z.pay[0] = FromInteger(pay[0][8]);
            z.pay[1] = FromInteger(pay[1][8]);
            z = outcomes[++zindex];
            nodes[95].father = 7;
            ...
            z.nodeIndex = 95;
            z.pay[0] = FromInteger(pay[0][9]);
            z.pay[1] = FromInteger(pay[1][9]);
            z = outcomes[++zindex];
            nodes[96].father = 10;
            ...
            z.nodeIndex = 96;
            z.pay[0] = FromInteger(pay[0][10]);
            z.pay[1] = FromInteger(pay[1][10]);
            z = outcomes[++zindex];
            nodes[97].father = 10;
            ...
            z.nodeIndex = 97;
            z.pay[0] = FromInteger(pay[0][11]);
            z.pay[1] = FromInteger(pay[1][11]);
            z = outcomes[++zindex];
            nodes[98].father = 10;
            ...
            z.nodeIndex = 98;
            z.pay[0] = FromInteger(pay[0][12]);
            z.pay[1] = FromInteger(pay[1][12]);
            z = outcomes[++zindex];
            nodes[99].father = 11;
            ...
            z.nodeIndex = 99;
            z.pay[0] = FromInteger(pay[0][13]);
            z.pay[1] = FromInteger(pay[1][13]);
            z = outcomes[++zindex];
            nodes[100].father = 11;
            ...
            z.nodeIndex = 100;
            z.pay[0] = FromInteger(pay[0][14]);
            z.pay[1] = FromInteger(pay[1][14]);
            z = outcomes[++zindex];
            nodes[101].father = 11;
            ...
            z.nodeIndex = 101;
            z.pay[0] = FromInteger(pay[0][15]);
            z.pay[1] = FromInteger(pay[1][15]);
            z = outcomes[++zindex];
            nodes[102].father = 9;
            ...
            z.nodeIndex = 102;
            z.pay[0] = FromInteger(pay[0][16]);
            z.pay[1] = FromInteger(pay[1][16]);
            z = outcomes[++zindex];
            nodes[103].father = 12;
            ...
            z.nodeIndex = 103;
            z.pay[0] = FromInteger(pay[0][17]);
            z.pay[1] = FromInteger(pay[1][17]);
            z = outcomes[++zindex];
            nodes[104].father = 13;
            ...
            z.nodeIndex = 104;
            z.pay[0] = FromInteger(pay[0][18]);
            z.pay[1] = FromInteger(pay[1][18]);
            z = outcomes[++zindex];
            nodes[105].father = 15;
            ...
            z.nodeIndex = 105;
            z.pay[0] = FromInteger(pay[0][19]);
            z.pay[1] = FromInteger(pay[1][19]);
            z = outcomes[++zindex];
            nodes[106].father = 15;
            ...
            z.nodeIndex = 106;
            z.pay[0] = FromInteger(pay[0][20]);
            z.pay[1] = FromInteger(pay[1][20]);
            z = outcomes[++zindex];
            nodes[107].father = 15;
            ...
            z.nodeIndex = 107;
            z.pay[0] = FromInteger(pay[0][21]);
            z.pay[1] = FromInteger(pay[1][21]);
            z = outcomes[++zindex];
            nodes[108].father = 17;
            ...
            z.nodeIndex = 108;
            z.pay[0] = FromInteger(pay[0][22]);
            z.pay[1] = FromInteger(pay[1][22]);
            z = outcomes[++zindex];
            nodes[109].father = 17;
            ...
            z.nodeIndex = 109;
            z.pay[0] = FromInteger(pay[0][23]);
            z.pay[1] = FromInteger(pay[1][23]);
            z = outcomes[++zindex];
            nodes[110].father = 17;
            ...
            z.nodeIndex = 110;
            z.pay[0] = FromInteger(pay[0][24]);
            z.pay[1] = FromInteger(pay[1][24]);
            z = outcomes[++zindex];
            nodes[111].father = 16;
            ...
            z.nodeIndex = 111;
            z.pay[0] = FromInteger(pay[0][25]);
            z.pay[1] = FromInteger(pay[1][25]);
            z = outcomes[++zindex];
            nodes[112].father = 16;
            ...
            z.nodeIndex = 112;
            z.pay[0] = FromInteger(pay[0][26]);
            z.pay[1] = FromInteger(pay[1][26]);
            z = outcomes[++zindex];
            nodes[113].father = 19;
            ...
            z.nodeIndex = 113;
            z.pay[0] = FromInteger(pay[0][27]);
            z.pay[1] = FromInteger(pay[1][27]);
            z = outcomes[++zindex];
            nodes[114].father = 19;
            ...
            z.nodeIndex = 114;
            z.pay[0] = FromInteger(pay[0][28]);
            z.pay[1] = FromInteger(pay[1][28]);
            z = outcomes[++zindex];
            nodes[115].father = 19;
            ...
            z.nodeIndex = 115;
            z.pay[0] = FromInteger(pay[0][29]);
            z.pay[1] = FromInteger(pay[1][29]);
            z = outcomes[++zindex];
            nodes[116].father = 20;
            ...
            z.nodeIndex = 116;
            z.pay[0] = FromInteger(pay[0][30]);
            z.pay[1] = FromInteger(pay[1][30]);
            z = outcomes[++zindex];
            nodes[117].father = 20;
            ...
            z.nodeIndex = 117;
            z.pay[0] = FromInteger(pay[0][31]);
            z.pay[1] = FromInteger(pay[1][31]);
            z = outcomes[++zindex];
            nodes[118].father = 20;
            ...
            z.nodeIndex = 118;
            z.pay[0] = FromInteger(pay[0][32]);
            z.pay[1] = FromInteger(pay[1][32]);
            z = outcomes[++zindex];
            nodes[119].father = 18;
            ...
            z.nodeIndex = 119;
            z.pay[0] = FromInteger(pay[0][33]);
            z.pay[1] = FromInteger(pay[1][33]);
            z = outcomes[++zindex];
            nodes[120].father = 21;
            ...
            z.nodeIndex = 120;
            z.pay[0] = FromInteger(pay[0][34]);
            z.pay[1] = FromInteger(pay[1][34]);
            z = outcomes[++zindex];
            nodes[121].father = 22;
            ...
            z.nodeIndex = 121;
            z.pay[0] = FromInteger(pay[0][35]);
            z.pay[1] = FromInteger(pay[1][35]);
            z = outcomes[++zindex];
            nodes[122].father = 24;
            ...
            z.nodeIndex = 122;
            z.pay[0] = FromInteger(pay[0][36]);
            z.pay[1] = FromInteger(pay[1][36]);
            z = outcomes[++zindex];
            nodes[123].father = 24;
            ...
            z.nodeIndex = 123;
            z.pay[0] = FromInteger(pay[0][37]);
            z.pay[1] = FromInteger(pay[1][37]);
            z = outcomes[++zindex];
            nodes[124].father = 24;
            ...
            z.nodeIndex = 124;
            z.pay[0] = FromInteger(pay[0][38]);
            z.pay[1] = FromInteger(pay[1][38]);
            z = outcomes[++zindex];
            nodes[125].father = 26;
            ...
            z.nodeIndex = 125;
            z.pay[0] = FromInteger(pay[0][39]);
            z.pay[1] = FromInteger(pay[1][39]);
            z = outcomes[++zindex];
            nodes[126].father = 26;
            ...
            z.nodeIndex = 126;
            z.pay[0] = FromInteger(pay[0][40]);
            z.pay[1] = FromInteger(pay[1][40]);
            z = outcomes[++zindex];
            nodes[127].father = 26;
            ...
            z.nodeIndex = 127;
            z.pay[0] = FromInteger(pay[0][41]);
            z.pay[1] = FromInteger(pay[1][41]);
            z = outcomes[++zindex];
            nodes[128].father = 25;
            ...
            z.nodeIndex = 128;
            z.pay[0] = FromInteger(pay[0][42]);
            z.pay[1] = FromInteger(pay[1][42]);
            z = outcomes[++zindex];
            nodes[129].father = 25;
            ...
            z.nodeIndex = 129;
            z.pay[0] = FromInteger(pay[0][43]);
            z.pay[1] = FromInteger(pay[1][43]);
            z = outcomes[++zindex];
            nodes[130].father = 28;
            ...
            z.nodeIndex = 130;
            z.pay[0] = FromInteger(pay[0][44]);
            z.pay[1] = FromInteger(pay[1][44]);
            z = outcomes[++zindex];
            nodes[131].father = 28;
            ...
            z.nodeIndex = 131;
            z.pay[0] = FromInteger(pay[0][45]);
            z.pay[1] = FromInteger(pay[1][45]);
            z = outcomes[++zindex];
            nodes[132].father = 28;
            ...
            z.nodeIndex = 132;
            z.pay[0] = FromInteger(pay[0][46]);
            z.pay[1] = FromInteger(pay[1][46]);
            z = outcomes[++zindex];
            nodes[133].father = 29;
            ...
            z.nodeIndex = 133;
            z.pay[0] = FromInteger(pay[0][47]);
            z.pay[1] = FromInteger(pay[1][47]);
            z = outcomes[++zindex];
            nodes[134].father = 29;
            ...
            z.nodeIndex = 134;
            z.pay[0] = FromInteger(pay[0][48]);
            z.pay[1] = FromInteger(pay[1][48]);
            z = outcomes[++zindex];
            nodes[135].father = 29;
            ...
            z.nodeIndex = 135;
            z.pay[0] = FromInteger(pay[0][49]);
            z.pay[1] = FromInteger(pay[1][49]);
            z = outcomes[++zindex];
            nodes[136].father = 27;
            ...
            z.nodeIndex = 136;
            z.pay[0] = FromInteger(pay[0][50]);
            z.pay[1] = FromInteger(pay[1][50]);
            z = outcomes[++zindex];
            nodes[137].father = 31;
            ...
            z.nodeIndex = 137;
            z.pay[0] = FromInteger(pay[0][51]);
            z.pay[1] = FromInteger(pay[1][51]);
            z = outcomes[++zindex];
            nodes[138].father = 32;
            ...
            z.nodeIndex = 138;
            z.pay[0] = FromInteger(pay[0][52]);
            z.pay[1] = FromInteger(pay[1][52]);
            z = outcomes[++zindex];
            nodes[139].father = 34;
            ...
            z.nodeIndex = 139;
            z.pay[0] = FromInteger(pay[0][53]);
            z.pay[1] = FromInteger(pay[1][53]);
            z = outcomes[++zindex];
            nodes[140].father = 34;
            ...
            z.nodeIndex = 140;
            z.pay[0] = FromInteger(pay[0][54]);
            z.pay[1] = FromInteger(pay[1][54]);
            z = outcomes[++zindex];
            nodes[141].father = 34;
            ...
            z.nodeIndex = 141;
            z.pay[0] = FromInteger(pay[0][55]);
            z.pay[1] = FromInteger(pay[1][55]);
            z = outcomes[++zindex];
            nodes[142].father = 36;
            ...
            z.nodeIndex = 142;
            z.pay[0] = FromInteger(pay[0][56]);
            z.pay[1] = FromInteger(pay[1][56]);
            z = outcomes[++zindex];
            nodes[143].father = 36;
            ...
            z.nodeIndex = 143;
            z.pay[0] = FromInteger(pay[0][57]);
            z.pay[1] = FromInteger(pay[1][57]);
            z = outcomes[++zindex];
            nodes[144].father = 36;
            ...
            z.nodeIndex = 144;
            z.pay[0] = FromInteger(pay[0][58]);
            z.pay[1] = FromInteger(pay[1][58]);
            z = outcomes[++zindex];
            nodes[145].father = 35;
            ...
            z.nodeIndex = 145;
            z.pay[0] = FromInteger(pay[0][59]);
            z.pay[1] = FromInteger(pay[1][59]);
            z = outcomes[++zindex];
            nodes[146].father = 35;
            ...
            z.nodeIndex = 146;
            z.pay[0] = FromInteger(pay[0][60]);
            z.pay[1] = FromInteger(pay[1][60]);
            z = outcomes[++zindex];
            nodes[147].father = 38;
            ...
            z.nodeIndex = 147;
            z.pay[0] = FromInteger(pay[0][61]);
            z.pay[1] = FromInteger(pay[1][61]);
            z = outcomes[++zindex];
            nodes[148].father = 38;
            ...
            z.nodeIndex = 148;
            z.pay[0] = FromInteger(pay[0][62]);
            z.pay[1] = FromInteger(pay[1][62]);
            z = outcomes[++zindex];
            nodes[149].father = 38;
            ...
            z.nodeIndex = 149;
            z.pay[0] = FromInteger(pay[0][63]);
            z.pay[1] = FromInteger(pay[1][63]);
            z = outcomes[++zindex];
            nodes[150].father = 39;
            ...
            z.nodeIndex = 150;
            z.pay[0] = FromInteger(pay[0][64]);
            z.pay[1] = FromInteger(pay[1][64]);
            z = outcomes[++zindex];
            nodes[151].father = 39;
            ...
            z.nodeIndex = 151;
            z.pay[0] = FromInteger(pay[0][65]);
            z.pay[1] = FromInteger(pay[1][65]);
            z = outcomes[++zindex];
            nodes[152].father = 39;
            ...
            z.nodeIndex = 152;
            z.pay[0] = FromInteger(pay[0][66]);
            z.pay[1] = FromInteger(pay[1][66]);
            z = outcomes[++zindex];
            nodes[153].father = 37;
            ...
            z.nodeIndex = 153;
            z.pay[0] = FromInteger(pay[0][67]);
            z.pay[1] = FromInteger(pay[1][67]);
            z = outcomes[++zindex];
            nodes[154].father = 40;
            ...
            z.nodeIndex = 154;
            z.pay[0] = FromInteger(pay[0][68]);
            z.pay[1] = FromInteger(pay[1][68]);
            z = outcomes[++zindex];
            nodes[155].father = 41;
            ...
            z.nodeIndex = 155;
            z.pay[0] = FromInteger(pay[0][69]);
            z.pay[1] = FromInteger(pay[1][69]);
            z = outcomes[++zindex];
            nodes[156].father = 43;
            ...
            z.nodeIndex = 156;
            z.pay[0] = FromInteger(pay[0][70]);
            z.pay[1] = FromInteger(pay[1][70]);
            z = outcomes[++zindex];
            nodes[157].father = 43;
            ...
            z.nodeIndex = 157;
            z.pay[0] = FromInteger(pay[0][71]);
            z.pay[1] = FromInteger(pay[1][71]);
            z = outcomes[++zindex];
            nodes[158].father = 43;
            ...
            z.nodeIndex = 158;
            z.pay[0] = FromInteger(pay[0][72]);
            z.pay[1] = FromInteger(pay[1][72]);
            z = outcomes[++zindex];
            nodes[159].father = 45;
            ...
            z.nodeIndex = 159;
            z.pay[0] = FromInteger(pay[0][73]);
            z.pay[1] = FromInteger(pay[1][73]);
            z = outcomes[++zindex];
            nodes[160].father = 45;
            ...
            z.nodeIndex = 160;
            z.pay[0] = FromInteger(pay[0][74]);
            z.pay[1] = FromInteger(pay[1][74]);
            z = outcomes[++zindex];
            nodes[161].father = 45;
            ...
            z.nodeIndex = 161;
            z.pay[0] = FromInteger(pay[0][75]);
            z.pay[1] = FromInteger(pay[1][75]);
            z = outcomes[++zindex];
            nodes[162].father = 44;
            ...
            z.nodeIndex = 162;
            z.pay[0] = FromInteger(pay[0][76]);
            z.pay[1] = FromInteger(pay[1][76]);
            z = outcomes[++zindex];
            nodes[163].father = 44;
            ...
            z.nodeIndex = 163;
            z.pay[0] = FromInteger(pay[0][77]);
            z.pay[1] = FromInteger(pay[1][77]);
            z = outcomes[++zindex];
            nodes[164].father = 47;
            ...
            z.nodeIndex = 164;
            z.pay[0] = FromInteger(pay[0][78]);
            z.pay[1] = FromInteger(pay[1][78]);
            z = outcomes[++zindex];
            nodes[165].father = 47;
            ...
            z.nodeIndex = 165;
            z.pay[0] = FromInteger(pay[0][79]);
            z.pay[1] = FromInteger(pay[1][79]);
            z = outcomes[++zindex];
            nodes[166].father = 47;
            ...
            z.nodeIndex = 166;
            z.pay[0] = FromInteger(pay[0][80]);
            z.pay[1] = FromInteger(pay[1][80]);
            z = outcomes[++zindex];
            nodes[167].father = 48;
            ...
            z.nodeIndex = 167;
            z.pay[0] = FromInteger(pay[0][81]);
            z.pay[1] = FromInteger(pay[1][81]);
            z = outcomes[++zindex];
            nodes[168].father = 48;
            ...
            z.nodeIndex = 168;
            z.pay[0] = FromInteger(pay[0][82]);
            z.pay[1] = FromInteger(pay[1][82]);
            z = outcomes[++zindex];
            nodes[169].father = 48;
            ...
            z.nodeIndex = 169;
            z.pay[0] = FromInteger(pay[0][83]);
            z.pay[1] = FromInteger(pay[1][83]);
            z = outcomes[++zindex];
            nodes[170].father = 46;
            ...
            z.nodeIndex = 170;
            z.pay[0] = FromInteger(pay[0][84]);
            z.pay[1] = FromInteger(pay[1][84]);
            z = outcomes[++zindex];
            nodes[171].father = 49;
            ...
            z.nodeIndex = 171;
            z.pay[0] = FromInteger(pay[0][85]);
            z.pay[1] = FromInteger(pay[1][85]);
            z = outcomes[++zindex];
            nodes[172].father = 50;
            ...
            z.nodeIndex = 172;
            z.pay[0] = FromInteger(pay[0][86]);
            z.pay[1] = FromInteger(pay[1][86]);
            z = outcomes[++zindex];
            nodes[173].father = 52;
            ...
            z.nodeIndex = 173;
            z.pay[0] = FromInteger(pay[0][87]);
            z.pay[1] = FromInteger(pay[1][87]);
            z = outcomes[++zindex];
            nodes[174].father = 52;
            ...
            z.nodeIndex = 174;
            z.pay[0] = FromInteger(pay[0][88]);
            z.pay[1] = FromInteger(pay[1][88]);
            z = outcomes[++zindex];
            nodes[175].father = 52;
            ...
            z.nodeIndex = 175;
            z.pay[0] = FromInteger(pay[0][89]);
            z.pay[1] = FromInteger(pay[1][89]);
            z = outcomes[++zindex];
            nodes[176].father = 54;
            ...
            z.nodeIndex = 176;
            z.pay[0] = FromInteger(pay[0][90]);
            z.pay[1] = FromInteger(pay[1][90]);
            z = outcomes[++zindex];
            nodes[177].father = 54;
            ...
            z.nodeIndex = 177;
            z.pay[0] = FromInteger(pay[0][91]);
            z.pay[1] = FromInteger(pay[1][91]);
            z = outcomes[++zindex];
            nodes[178].father = 54;
            ...
            z.nodeIndex = 178;
            z.pay[0] = FromInteger(pay[0][92]);
            z.pay[1] = FromInteger(pay[1][92]);
            z = outcomes[++zindex];
            nodes[179].father = 53;
            ...
            z.nodeIndex = 179;
            z.pay[0] = FromInteger(pay[0][93]);
            z.pay[1] = FromInteger(pay[1][93]);
            z = outcomes[++zindex];
            nodes[180].father = 53;
            ...
            z.nodeIndex = 180;
            z.pay[0] = FromInteger(pay[0][94]);
            z.pay[1] = FromInteger(pay[1][94]);
            z = outcomes[++zindex];
            nodes[181].father = 56;
            ...
            z.nodeIndex = 181;
            z.pay[0] = FromInteger(pay[0][95]);
            z.pay[1] = FromInteger(pay[1][95]);
            z = outcomes[++zindex];
            nodes[182].father = 56;
            ...
            z.nodeIndex = 182;
            z.pay[0] = FromInteger(pay[0][96]);
            z.pay[1] = FromInteger(pay[1][96]);
            z = outcomes[++zindex];
            nodes[183].father = 56;
            ...
            z.nodeIndex = 183;
            z.pay[0] = FromInteger(pay[0][97]);
            z.pay[1] = FromInteger(pay[1][97]);
            z = outcomes[++zindex];
            nodes[184].father = 57;
            ...
            z.nodeIndex = 184;
            z.pay[0] = FromInteger(pay[0][98]);
            z.pay[1] = FromInteger(pay[1][98]);
            z = outcomes[++zindex];
            nodes[185].father = 57;
            ...
            z.nodeIndex = 185;
            z.pay[0] = FromInteger(pay[0][99]);
            z.pay[1] = FromInteger(pay[1][99]);
            z = outcomes[++zindex];
            nodes[186].father = 57;
            ...
            z.nodeIndex = 186;
            z.pay[0] = FromInteger(pay[0][100]);
            z.pay[1] = FromInteger(pay[1][100]);
            z = outcomes[++zindex];
            nodes[187].father = 55;
            ...
            z.nodeIndex = 187;
            z.pay[0] = FromInteger(pay[0][101]);
            z.pay[1] = FromInteger(pay[1][101]);
            z = outcomes[++zindex];
            nodes[188].father = 59;
            ...
            z.nodeIndex = 188;
            z.pay[0] = FromInteger(pay[0][102]);
            z.pay[1] = FromInteger(pay[1][102]);
            z = outcomes[++zindex];
            nodes[189].father = 60;
            ...
            z.nodeIndex = 189;
            z.pay[0] = FromInteger(pay[0][103]);
            z.pay[1] = FromInteger(pay[1][103]);
            z = outcomes[++zindex];
            nodes[190].father = 62;
            ...
            z.nodeIndex = 190;
            z.pay[0] = FromInteger(pay[0][104]);
            z.pay[1] = FromInteger(pay[1][104]);
            z = outcomes[++zindex];
            nodes[191].father = 62;
            ...
            z.nodeIndex = 191;
            z.pay[0] = FromInteger(pay[0][105]);
            z.pay[1] = FromInteger(pay[1][105]);
            z = outcomes[++zindex];
            nodes[192].father = 62;
            ...
            z.nodeIndex = 192;
            z.pay[0] = FromInteger(pay[0][106]);
            z.pay[1] = FromInteger(pay[1][106]);
            z = outcomes[++zindex];
            nodes[193].father = 64;
            ...
            z.nodeIndex = 193;
            z.pay[0] = FromInteger(pay[0][107]);
            z.pay[1] = FromInteger(pay[1][107]);
            z = outcomes[++zindex];
            nodes[194].father = 64;
            ...
            z.nodeIndex = 194;
            z.pay[0] = FromInteger(pay[0][108]);
            z.pay[1] = FromInteger(pay[1][108]);
            z = outcomes[++zindex];
            nodes[195].father = 64;
            ...
            z.nodeIndex = 195;
            z.pay[0] = FromInteger(pay[0][109]);
            z.pay[1] = FromInteger(pay[1][109]);
            z = outcomes[++zindex];
            nodes[196].father = 63;
            ...
            z.nodeIndex = 196;
            z.pay[0] = FromInteger(pay[0][110]);
            z.pay[1] = FromInteger(pay[1][110]);
            z = outcomes[++zindex];
            nodes[197].father = 63;
            ...
            z.nodeIndex = 197;
            z.pay[0] = FromInteger(pay[0][111]);
            z.pay[1] = FromInteger(pay[1][111]);
            z = outcomes[++zindex];
            nodes[198].father = 66;
            ...
            z.nodeIndex = 198;
            z.pay[0] = FromInteger(pay[0][112]);
            z.pay[1] = FromInteger(pay[1][112]);
            z = outcomes[++zindex];
            nodes[199].father = 66;
            ...
            z.nodeIndex = 199;
            z.pay[0] = FromInteger(pay[0][113]);
            z.pay[1] = FromInteger(pay[1][113]);
            z = outcomes[++zindex];
            nodes[200].father = 66;
            ...
            z.nodeIndex = 200;
            z.pay[0] = FromInteger(pay[0][114]);
            z.pay[1] = FromInteger(pay[1][114]);
            z = outcomes[++zindex];
            nodes[201].father = 67;
            ...
            z.nodeIndex = 201;
            z.pay[0] = FromInteger(pay[0][115]);
            z.pay[1] = FromInteger(pay[1][115]);
            z = outcomes[++zindex];
            nodes[202].father = 67;
            ...
            z.nodeIndex = 202;
            z.pay[0] = FromInteger(pay[0][116]);
            z.pay[1] = FromInteger(pay[1][116]);
            z = outcomes[++zindex];
            nodes[203].father = 67;
            ...
            z.nodeIndex = 203;
            z.pay[0] = FromInteger(pay[0][117]);
            z.pay[1] = FromInteger(pay[1][117]);
            z = outcomes[++zindex];
            nodes[204].father = 65;
            ...
            z.nodeIndex = 204;
            z.pay[0] = FromInteger(pay[0][118]);
            z.pay[1] = FromInteger(pay[1][118]);
            z = outcomes[++zindex];
            nodes[205].father = 68;
            ...
            z.nodeIndex = 205;
            z.pay[0] = FromInteger(pay[0][119]);
            z.pay[1] = FromInteger(pay[1][119]);
            z = outcomes[++zindex];
            nodes[206].father = 69;
            ...
            z.nodeIndex = 206;
            z.pay[0] = FromInteger(pay[0][120]);
            z.pay[1] = FromInteger(pay[1][120]);
            z = outcomes[++zindex];
            nodes[207].father = 71;
            ...
            z.nodeIndex = 207;
            z.pay[0] = FromInteger(pay[0][121]);
            z.pay[1] = FromInteger(pay[1][121]);
            z = outcomes[++zindex];
            nodes[208].father = 71;
            ...
            z.nodeIndex = 208;
            z.pay[0] = FromInteger(pay[0][122]);
            z.pay[1] = FromInteger(pay[1][122]);
            z = outcomes[++zindex];
            nodes[209].father = 71;
            ...
            z.nodeIndex = 209;
            z.pay[0] = FromInteger(pay[0][123]);
            z.pay[1] = FromInteger(pay[1][123]);
            z = outcomes[++zindex];
            nodes[210].father = 73;
            ...
            z.nodeIndex = 210;
            z.pay[0] = FromInteger(pay[0][124]);
            z.pay[1] = FromInteger(pay[1][124]);
            z = outcomes[++zindex];
            nodes[211].father = 73;
            ...
            z.nodeIndex = 211;
            z.pay[0] = FromInteger(pay[0][125]);
            z.pay[1] = FromInteger(pay[1][125]);
            z = outcomes[++zindex];
            nodes[212].father = 73;
            ...
            z.nodeIndex = 212;
            z.pay[0] = FromInteger(pay[0][126]);
            z.pay[1] = FromInteger(pay[1][126]);
            z = outcomes[++zindex];
            nodes[213].father = 72;
            ...
            z.nodeIndex = 213;
            z.pay[0] = FromInteger(pay[0][127]);
            z.pay[1] = FromInteger(pay[1][127]);
            z = outcomes[++zindex];
            nodes[214].father = 72;
            ...
            z.nodeIndex = 214;
            z.pay[0] = FromInteger(pay[0][128]);
            z.pay[1] = FromInteger(pay[1][128]);
            z = outcomes[++zindex];
            nodes[215].father = 75;
            ...
            z.nodeIndex = 215;
            z.pay[0] = FromInteger(pay[0][129]);
            z.pay[1] = FromInteger(pay[1][129]);
            z = outcomes[++zindex];
            nodes[216].father = 75;
            ...
            z.nodeIndex = 216;
            z.pay[0] = FromInteger(pay[0][130]);
            z.pay[1] = FromInteger(pay[1][130]);
            z = outcomes[++zindex];
            nodes[217].father = 75;
            ...
            z.nodeIndex = 217;
            z.pay[0] = FromInteger(pay[0][131]);
            z.pay[1] = FromInteger(pay[1][131]);
            z = outcomes[++zindex];
            nodes[218].father = 76;
            ...
            z.nodeIndex = 218;
            z.pay[0] = FromInteger(pay[0][132]);
            z.pay[1] = FromInteger(pay[1][132]);
            z = outcomes[++zindex];
            nodes[219].father = 76;
            ...
            z.nodeIndex = 219;
            z.pay[0] = FromInteger(pay[0][133]);
            z.pay[1] = FromInteger(pay[1][133]);
            z = outcomes[++zindex];
            nodes[220].father = 76;
            ...
            z.nodeIndex = 220;
            z.pay[0] = FromInteger(pay[0][134]);
            z.pay[1] = FromInteger(pay[1][134]);
            z = outcomes[++zindex];
            nodes[221].father = 74;
            ...
            z.nodeIndex = 221;
            z.pay[0] = FromInteger(pay[0][135]);
            z.pay[1] = FromInteger(pay[1][135]);
            z = outcomes[++zindex];
            nodes[222].father = 77;
            ...
            z.nodeIndex = 222;
            z.pay[0] = FromInteger(pay[0][136]);
            z.pay[1] = FromInteger(pay[1][136]);
            z = outcomes[++zindex];
            nodes[223].father = 78;
            ...
            z.nodeIndex = 223;
            z.pay[0] = FromInteger(pay[0][137]);
            z.pay[1] = FromInteger(pay[1][137]);
            z = outcomes[++zindex];
            nodes[224].father = 80;
            ...
            z.nodeIndex = 224;
            z.pay[0] = FromInteger(pay[0][138]);
            z.pay[1] = FromInteger(pay[1][138]);
            z = outcomes[++zindex];
            nodes[225].father = 80;
            ...
            z.nodeIndex = 225;
            z.pay[0] = FromInteger(pay[0][139]);
            z.pay[1] = FromInteger(pay[1][139]);
            z = outcomes[++zindex];
            nodes[226].father = 80;
            ...
            z.nodeIndex = 226;
            z.pay[0] = FromInteger(pay[0][140]);
            z.pay[1] = FromInteger(pay[1][140]);
            z = outcomes[++zindex];
            nodes[227].father = 82;
            ...
            z.nodeIndex = 227;
            z.pay[0] = FromInteger(pay[0][141]);
            z.pay[1] = FromInteger(pay[1][141]);
            z = outcomes[++zindex];
            nodes[228].father = 82;
            ...
            z.nodeIndex = 228;
            z.pay[0] = FromInteger(pay[0][142]);
            z.pay[1] = FromInteger(pay[1][142]);
            z = outcomes[++zindex];
            nodes[229].father = 82;
            ...
            z.nodeIndex = 229;
            z.pay[0] = FromInteger(pay[0][143]);
            z.pay[1] = FromInteger(pay[1][143]);
            z = outcomes[++zindex];
            nodes[230].father = 81;
            ...
            z.nodeIndex = 230;
            z.pay[0] = FromInteger(pay[0][144]);
            z.pay[1] = FromInteger(pay[1][144]);
            z = outcomes[++zindex];
            nodes[231].father = 81;
            ...
            z.nodeIndex = 231;
            z.pay[0] = FromInteger(pay[0][145]);
            z.pay[1] = FromInteger(pay[1][145]);
            z = outcomes[++zindex];
            nodes[232].father = 84;
            ...
            z.nodeIndex = 232;
            z.pay[0] = FromInteger(pay[0][146]);
            z.pay[1] = FromInteger(pay[1][146]);
            z = outcomes[++zindex];
            nodes[233].father = 84;
            ...
            z.nodeIndex = 233;
            z.pay[0] = FromInteger(pay[0][147]);
            z.pay[1] = FromInteger(pay[1][147]);
            z = outcomes[++zindex];
            nodes[234].father = 84;
            ...
            z.nodeIndex = 234;
            z.pay[0] = FromInteger(pay[0][148]);
            z.pay[1] = FromInteger(pay[1][148]);
            z = outcomes[++zindex];
            nodes[235].father = 85;
            ...
            z.nodeIndex = 235;
            z.pay[0] = FromInteger(pay[0][149]);
            z.pay[1] = FromInteger(pay[1][149]);
            z = outcomes[++zindex];
            nodes[236].father = 85;
            ...
            z.nodeIndex = 236;
            z.pay[0] = FromInteger(pay[0][150]);
            z.pay[1] = FromInteger(pay[1][150]);
            z = outcomes[++zindex];
            nodes[237].father = 85;
            ...
            z.nodeIndex = 237;
            z.pay[0] = FromInteger(pay[0][151]);
            z.pay[1] = FromInteger(pay[1][151]);
            z = outcomes[++zindex];
            nodes[238].father = 83;
            ...
            z.nodeIndex = 238;
            z.pay[0] = FromInteger(pay[0][152]);
            z.pay[1] = FromInteger(pay[1][152]);
            zindex++;
            nodes[1].iset = 0;
            nodes[2].iset = 1;
            nodes[3].iset = 3;
            ...
            moves[1].behavioralProbability = 1/(Rational)3;
            moves[2].priorInformationSet = 0;
            moves[2].behavioralProbability = 1 / (Rational)3;
            moves[3].priorInformationSet = 0;
            moves[3].behavioralProbability = 1 / (Rational)3;
            moves[4].priorInformationSet = 1;
            moves[4].behavioralProbability = 1 / (Rational)3;
            moves[5].priorInformationSet = 1;
            moves[5].behavioralProbability = 1 / (Rational)3;
            moves[6].priorInformationSet = 1;
            moves[6].behavioralProbability = 1 / (Rational)3;
            moves[7].priorInformationSet = 2;
            moves[7].behavioralProbability = 1 / (Rational)3;
            moves[8].priorInformationSet = 2;
            moves[8].behavioralProbability = 1 / (Rational)3;
            moves[9].priorInformationSet = 2;
            moves[9].behavioralProbability = 1 / (Rational)3;
