...
public static class StrategicCache
{
    ...
    public static void Import(string bundle,BundleManifest manifest,ResolvedArticlePlan plan,Dictionary<string,(JsonObject Audit,JsonObject Profile)> profiles,string output)
    {
        ...
        var pairs=manifest.Strategic.Length>0?manifest.Strategic:manifest.Renders.Where(r=>r.Output.StartsWith("Supplemental materials/Equilibrium strategy changes/")).GroupBy(r=>Path.GetDirectoryName(r.Source)!).Select(g=>new StrategicInput(g.Key,g.First().CaseIds)).ToArray();
        foreach(var entry in pairs.Where(p=>p.CaseIds.All(profiles.ContainsKey)))
        {
            if(entry.CaseIds.Length!=2)throw new InvalidDataException("Unbound strategic cache.");
            ...
            string dir=Files.Under(bundle,entry.Directory);var receipt=Files.Object(Path.Combine(dir,"validation.json"));var checks=receipt["Checks"]!;
            ...
            string id=manifest.Strategic.Length>0?new DirectoryInfo(dir).Name:new DirectoryInfo(Path.GetDirectoryName(dir)!).Name;string target=Path.Combine(output,id);Directory.CreateDirectory(target);
