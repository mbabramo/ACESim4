...
An optional container can provide the same tested environment. Keep a versioned container recipe with the code, pin its dependencies, and run the same C# entry point. Tool installations belong inside the container image, not in the mounted article directory. Mount proposed computational inputs read-only and a separate output directory writable. The container is an additional deployment option, not a requirement for native Windows/Linux replication, and must pass the same scientific and collection checks. No working container or Linux compatibility is claimed yet.
...
The existing clean-build tests establish saved-profile revalidation, native main Figures 1–7 and Tables 1/5, complete standard LitigCharts coverage and selected-stage execution. Rendering.cs still copies remaining data-bearing custom TeX from the old solutions bundle. Bundle.cs still packages authored inputs and rendered-layout inputs together with computation. Manuscript.cs copies authored source from that bundle. Source snapshots now work without Git; installation instructions, prerequisite checks and an MSBuild-driven container are implemented. Linux validation is recorded separately from Windows evidence. Multiple-start/tremble and fresh-computation stages are not connected. The remaining integrations and computation-only bundle must be completed before claiming this contract is satisfied. The successful 685-standard-diagram check does not close those gaps.
